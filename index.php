<?php
include_once "header.php";
?>
        <!--header-->
  <h1 class="cali">Bienvenidos a Miner.IA</h1>  
    <div id="carouselExampleSlidesOnly" class="carousel slide" data-bs-ride="carousel">
      <div class="carousel-inner">
       <div class="carousel-item active">
        <img src="img/camion.jpg" class="d-block w-100" alt="placeholder" width="1000" height="700">
    </div>
       <div class="carousel-item ">
        <img src="img/oro.jpg" class="d-block w-100" alt="placeholder" width="1000" height="500">
    </div>
       <div class="carousel-item ">
        <img src="img/lapis.jpg" class="d-block w-100" alt="placeholder" width="1000" height="700">
    </div>
  </div>
</div>

        <!--Cards para Our Partners-->
<main class="container-fluid">
  <div class="row">
    
     <div class="col-6">
      <div class="row"> 
        <div class="col">
          <h2 class="title-idx">About Us</h2>
          <p class="col">Lorem ipsum dolor sit amet consectetur adipisicing elit. Atque nobis ea ad quam omnis? Numquam magnam itaque quia facilis accusantium alias vel, nam temporibus nihil, fuga iusto vitae est cupiditate!
          Omnis enim placeat rem eaque, quidem officiis inventore facilis laboriosam totam assumenda, at reiciendis quibusdam deleniti corporis maiores quae nulla. Mollitia, quasi. Molestiae modi soluta minima? Nostrum dolorum sed dignissimos.
          Quae, illo deserunt at laboriosam dolorem voluptatibus similique eligendi delectus ad ipsam veritatis quisquam quas et id tempore aliquam error quidem exercitationem necessitatibus officiis deleniti! Sequi exercitationem debitis amet neque?</p>
          <div class="col">
            <img src="img/Logo_MinerIA.png" alt="Miner.IA" width="900" height="500">
          </div> 
          <p class="col">Lorem ipsum, dolor sit amet consectetur adipisicing elit. Perferendis non nostrum, voluptatibus quia accusantium ipsum alias, labore optio nesciunt recusandae est facere ratione porro odit qui, at praesentium doloremque magni. Lorem ipsum dolor sit amet consectetur adipisicing elit. Earum dignissimos reprehenderit praesentium, aliquam corporis obcaecati nam. Repudiandae, sint. Ipsam vitae corporis repellendus, nulla repellat nam. Qui exercitationem iure unde id. Lorem ipsum dolor sit, amet consectetur adipisicing elit. Ex, alias nostrum. Pariatur aliquam harum quis, quae iure deserunt nemo ipsa, iusto eius labore laboriosam rerum, ex veniam vitae officia accusantium.</p>
        </div>  
      </div>  
     </div> 
      
      <div class="col-6">
            <!--Primeras Cards-->
            <h2 class="title-idx">Our Partners</h2>
          <div class="row">
            <div class="col-6">
                <div class="card partners" style="width: 18rem;">
                      <img src="img/axion.jpg" class="card-img-top" alt="axion" width="276" height="276" >
                    <div class="card-body">
                      <p class="card-text">Some quick example text to build on the card title and make up the bulk of the card's content.</p>
                    </div>
               </div>            
 <br>
                <div class="card partners" style="width: 18rem;" >
                    <img src="img/home.png" class="card-img-top" alt="home" width="276" height="276">
                  <div class="card-body">
                    <p class="card-text">Some quick example text to build on the card title and make up the bulk of the card's content.</p>
                  </div>
                </div>
              </div>
          
 <br>
              <!--Otras dos cards-->
          
            <div class="col-6">
               <div class="card partners" style="width: 18rem;" >
                    <img src="img/tesla.png" class="card-img-top" alt="solar" width="276" height="276">
                  <div class="card-body">
                    <p class="card-text">Some quick example text to build on the card title and make up the bulk of the card's content.</p>
                  </div>
               </div>
            
 <br>
            
                <div class="card partners" style="width: 18rem;" >
                    <img src="img/nation.png" class="card-img-top" alt="national" width="276" height="276">
                  <div class="card-body">
                    <p class="card-text">Some quick example text to build on the card title and make up the bulk of the card's content.</p>
                  </div>
                </div>
                </div>
          </div>





      </div>
  </div>

</main>








<br>






         
         <!--footer-->
<?php
include_once "footer.php";
?>         
        